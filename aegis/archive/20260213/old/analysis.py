...
DEFAULT_MANAGERS = [
    ...
    {"name": "Steve Cooper", "search": "Steve Cooper"},
    {"name": "David Moyes", "search": "David Moyes"},
...
class ManagerDNATrainer:
    ...
    def __init__(
        ...
        Args:
            ...
            season_id: Season to analyse. If None, auto-detects most recent season with data (default: None)
        """
        self.client = client
        self.training_dir = Path(training_dir) if training_dir else Config.PROCESSED_DIR / "training"
        self.season_id = season_id if season_id else 23614  # Default fallback
    ...
    def fetch_manager_data(
        ...
        for mgr in managers:
            try:
                ...
                # Find current or most recent team
                current_team = None
                for team in teams:
                    if team.get("end") is None:
                        current_team = team
                        break
                if not current_team and teams:
    ...
    def extract_features(self, verbose: bool = True) -> "ManagerDNATrainer":
        ...
        If season_id is None, fetches data from ALL available seasons (2024/25, 2023/24, 2022/23).
        This creates multiple entries per manager (one per season), maximizing training data.
        ...
        if self.season_id is None:
            # Fetch ALL seasons
            seasons_to_fetch = [23614, 21646, 19735]  # 2024/25, 2023/24, 2022/23
            fetch_mode = "ALL"
            if verbose:
                print("Mode: Fetching ALL seasons (maximizes training data)")
        else:
            # Fetch only specified season
            seasons_to_fetch = [self.season_id]
            fetch_mode = "SINGLE"
            season_label = {23614: "2024/25", 21646: "2023/24", 19735: "2022/23"}.get(self.season_id, str(self.season_id))
            if verbose:
                print(f"Mode: Fetching {season_label} only")
        ...
        for tenure in self.manager_tenures:
            ...
            for season_id in seasons_to_fetch:
                try:
                    ...
                    if not team_stats or not team_stats.get("statistics"):
                        continue
                    ...
                    if features:
                        ...
                        if verbose:
                            season_label = {23614: "2024/25", 21646: "2023/24", 19735: "2022/23"}.get(season_id, str(season_id))
                            print(f"  * {tenure['coach_name']}: {len(MANAGER_DNA_FEATURES)} features ({season_label})")
                    
                except Exception as e:
                    # Silently skip seasons with errors
                    continue
        
        if verbose:
            print("-" * 60)
            print(f"Total data points: {len(self.manager_features)}")
            
            # Show season distribution
            from collections import Counter
            season_counts = Counter(f["season_id"] for f in self.manager_features)
            if season_counts:
                print("\nData by season:")
                for season_id, count in sorted(season_counts.items(), reverse=True):
                    season_label = {23614: "2024/25", 21646: "2023/24", 19735: "2022/23"}.get(season_id, str(season_id))
                    print(f"  {season_label}: {count} data points")
            
            # Show unique managers
            unique_managers = len(set(f["coach_name"] for f in self.manager_features))
            print(f"\nUnique managers: {unique_managers}")
            if fetch_mode == "ALL":
                print(f"Average seasons per manager: {len(self.manager_features) / unique_managers:.1f}")
    ...
    def _parse_team_stats(self, team_data: Dict) -> Optional[Dict]:
        ...
        # Possession
        poss = parsed.get("ball-possession", {})
        features["possession"] = poss.get("all", {}).get("average", 50)
        
        # Pass accuracy
        pass_pct = parsed.get("passes-percentage", {})
        features["pass_accuracy"] = pass_pct.get("all", {}).get("average", 80)
        
        # Shots
        shots = parsed.get("shots-total", {})
        features["shots"] = shots.get("all", {}).get("average", 12)
        
        # Goals scored
        goals = parsed.get("goals", {})
        features["goals_scored"] = goals.get("all", {}).get("average", 1.5)
        
        # Goals conceded
        conceded = parsed.get("goals-conceded", parsed.get("goals-condeded", {}))
        features["goals_conceded"] = conceded.get("all", {}).get("average", 1.2)
        
        # Tackles
        tackles = parsed.get("tackles", {})
        features["tackles"] = tackles.get("all", {}).get("average", 15)
        
        # Interceptions
        interceptions = parsed.get("interceptions", {})
        features["interceptions"] = interceptions.get("all", {}).get("average", 10)
        
        # Clean sheet percentage
        clean = parsed.get("cleansheets", {})
        features["clean_sheet_pct"] = clean.get("all", {}).get("percentage", 30)
        
        # Win rate
        wins = parsed.get("wins", {})
        features["win_rate"] = wins.get("all", {}).get("percentage", 33)
    ...
    def _name_clusters(self, verbose: bool = True):
        """Auto-generate cluster names based on most distinctive features."""
        import pandas as pd
        
        overall_mean = self.df_managers[MANAGER_DNA_FEATURES].mean()
        overall_std = self.df_managers[MANAGER_DNA_FEATURES].std()
        
        # Feature to archetype name mapping (positive direction)
        feature_names_positive = {
            "possession": "Possession",
            "pass_accuracy": "Technical",
            "shots": "Shot-Heavy",
            "goals_scored": "High-Scoring",
            "goals_conceded": "Defensive",  # LOW conceded = defensive (inverted)
            "tackles": "Pressing",
        ...
        # Feature to archetype name mapping (negative direction)
        feature_names_negative = {
            "possession": "Direct",
            "pass_accuracy": "Physical",
            "shots": "Clinical",
            "goals_scored": "Low-Scoring",
            "goals_conceded": "High-Risk",  # HIGH conceded = vulnerable
            "tackles": "Passive",
            "interceptions": "Reactive",
            "clean_sheet_pct": "Expansive",
            "win_rate": "Struggling"
        }
        
        # Negative traits (lower = better, so we invert for z-score)
        ...
        for c in range(self.n_clusters):
            ...
            for feat in MANAGER_DNA_FEATURES:
                if overall_std[feat] > 0:
                    ...
                    # Invert negative traits so "low goals conceded" becomes positive
            ...
            # Find top 2 most distinctive features (highest absolute z-scores)
            sorted_features = sorted(z_scores.items(), key=lambda x: abs(x[1]), reverse=True)
            
            top_feat, top_z = sorted_features[0]
            second_feat, second_z = sorted_features[1]
            
            # Build name based on distinctive features
            if top_z > 0.5:
                # Strong positive trait
                if top_feat == "possession" and z_scores.get("pass_accuracy", 0) > 0.3:
                    name = "Possession-Based"
                elif top_feat in ["tackles", "interceptions"]:
                    name = "High-Press"
                elif top_feat in ["shots", "goals_scored"]:
                    name = "Attacking"
                elif top_feat == "goals_conceded" or top_feat == "clean_sheet_pct":
                    name = "Defensive"
                    ...
                    name = feature_names_positive.get(top_feat, "Balanced")
                    
            elif top_z < -0.5:
                # Strong negative trait
                if top_feat == "possession":
                    name = "Counter-Attack"
                elif top_feat in ["tackles", "interceptions"]:
                    name = "Low-Block"
                elif top_feat == "goals_conceded":
                    name = "High-Risk"  # Concede lots = attacking but vulnerable
                elif top_feat == "clean_sheet_pct":
                    name = "Expansive"
                else:
                    name = feature_names_negative.get(top_feat, "Pragmatic")
            else:
                # No strong distinctive feature - use moderate traits
                if abs(top_z) > 0.2:
                    if top_z > 0:
                        name = feature_names_positive.get(top_feat, "Balanced")
                    else:
                        name = feature_names_negative.get(top_feat, "Balanced")
                else:
                    name = "Balanced"
            ...
            while name in used_names:
                suffix += 1
                # Add secondary trait to differentiate
                if suffix == 2 and second_feat and abs(second_z) > 0.2:
                    if second_z > 0:
                        secondary = feature_names_positive.get(second_feat, "")
                    else:
                        secondary = feature_names_negative.get(second_feat, "")
                    if secondary:
    ...
    def save(self, verbose: bool = True) -> "ManagerDNATrainer":
        ...
        if verbose:
            ...
            print("MODEL SAVED")
            print("=" * 60)
            print(f"  * {self.training_dir}/manager_dna_model.pkl")
            print(f"  * {self.training_dir}/manager_profiles.csv")
            print(f"  * {self.training_dir}/cluster_centroids.csv")
            print(f"  * {self.training_dir}/manager_dna_summary.json")
...
class SquadFitAnalyzer:
    ...
    def fetch_squad(self, club_name: str, verbose: bool = True) -> "SquadFitAnalyzer":
        """
        Fetch target club's squad with full player statistics.
        
        Note: This fetches each player individually to ensure statistics are populated,
        as get_squad() often returns empty details arrays.
        """
        from .client import SportsmonksClient
        ...
        # Fetch squad (basic player list)
        squad = self.client.get_squad(team_id, self.season_id)
        
        if verbose:
            print(f"  * {self.target_club}: {len(squad)} players")
            print(f"  * Fetching detailed statistics for each player...")
        
        # Enrich each player with full statistics
        # This is necessary because get_squad() often has empty details arrays
        enriched_squad = []
        errors = 0
        ...
        for i, entry in enumerate(squad, 1):
            ...
            if not player_id:
                enriched_squad.append(entry)
                continue
            
            try:
                # Fetch full player data with statistics
                player_full = self.client.get_player(player_id)
                
                # Replace player data with enriched version
                enriched_entry = entry.copy()
                enriched_entry["player"] = player_full
                enriched_squad.append(enriched_entry)
                
            except Exception as e:
                errors += 1
                if verbose and errors <= 3:
                    print(f"    ⚠ Error fetching {player.get('common_name', 'Unknown')}: {e}")
                # Keep original entry even if fetch fails
                enriched_squad.append(entry)
        
        self.target_squad = enriched_squad
        
        if verbose:
            if errors > 0:
                print(f"  ⚠ {errors} players had errors (using partial data)")
            print(f"  * Squad ready for analysis")
    ...
    def _get_position_group(self, player: Dict) -> str:
        ...
        if "goal" in position_lower:
            return "GK"
        elif any(p.lower() in position_lower for p in ["back", "defender", "centre-back"]):
            return "DEF"
        elif any(p.lower() in position_lower for p in ["winger", "wing", "forward", "striker", "attacker"]):
            return "ATT"
        else:
            return "MID"
    ...
    def _generate_ideal_xi(self, verbose: bool = True):
        """Generate ideal starting XI based on fit scores."""
        formation_slots = {
            ...
            "DM": {"group": "MID", "pref": ["Defensive Midfield"]},
            "CM": {"group": "MID", "pref": ["Central Midfield", "Midfielder"]},
            "AM": {"group": "MID", "pref": ["Attacking Midfield"]},
        ...
        for slot, config in formation_slots.items():
            ...
            if "pref" in config and candidates:
                preferred = [
                    ...
                    if any(pref.lower() in (p.detailed_position or p.position).lower() for pref in config["pref"])
                ]
                if preferred:
        ...
        if verbose and self.ideal_xi:
            ...
            print(f"\nâœ“ Ideal XI average fit: {avg_fit:.1f}")
    ...
    def save(self, verbose: bool = True) -> "SquadFitAnalyzer":
        ...
        if verbose:
            ...
            print("RESULTS SAVED")
            print("=" * 60)
            print(f"  * {self.output_dir}/squad_fit_scores.csv")
            print(f"  * {self.output_dir}/ideal_xi.csv")
            print(f"  * {self.output_dir}/squad_fit_summary.json")
