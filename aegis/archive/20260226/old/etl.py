...
class ETLPipeline:
    ...
    def _extract_dna_from_aggregated(self):
        ...
        print(f"      âœ“ Win rate: {self.manager_dna['results_profile']['win_rate']:.1f}%")
        print(f"      âœ“ Home: {self.manager_dna['results_profile']['win_rate_home']:.1f}% | Away: {self.manager_dna['results_profile']['win_rate_away']:.1f}%")
        ...
        for fixture in self.fixtures:
            ...
            for stat in stats:
                if stat.get("participant_id") == team_id or stat.get("location") == "home":
                    ...
                    value = stat.get("data", {}).get("value", 0) if isinstance(stat.get("data"), dict) else 0
