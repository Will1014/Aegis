...
class ManagerDNATrainer:
    ...
    def extract_features(self, verbose: bool = True) -> "ManagerDNATrainer":
        ...
        Two data sources (confirmed by API diagnostic Feb 2026):
          ...
          - Fixture stats:       provides pass_accuracy (not in team stats endpoint)
        ...
        for tenure in self.manager_tenures:
            try:
                ...
                # Step 2: pass_accuracy from fixture stats
                # The team stats endpoint has no pass accuracy code.
    ...
    def _fetch_pass_accuracy_from_fixtures(
        ...
        """
        Average pass accuracy across completed fixtures.

        The team stats endpoint does not provide a pass accuracy figure.
        At fixture level it lives as:
            stat["type"]["code"]  == "successful-passes-percentage"
            stat["participant_id"] == team_id
            stat["data"]["value"] == numeric value  (e.g. 85)

        Requires include=statistics.type on the fixture request.

        Returns:
            Season average pass accuracy (0-100), or default if unavailable.
        """
        try:
        ...
        if not values:
            if verbose:
                print(f"    ⚠  No pass accuracy values found in fixtures, using default {default}")
            return default
    ...
    def _parse_team_stats(self, team_data: Dict) -> Optional[Dict]:
        ...
        The team stats endpoint uses include=statistics.details.type which
        returns type as a dict with a "code" key.

        Confirmed codes and paths:
          possession      "ball-possession"    value["average"]                    flat
          shots           "shots"              value["average"]                    flat
          tackles         "tackles"            value["average"]                    flat
          interceptions   "interception-stats" value["interceptions_per_game"]     flat
          goals_scored    "goals"              value["all"]["average"]             nested
          goals_conceded  "goals-conceded"     value["all"]["average"]             nested
          clean_sheet_pct "cleansheets"        value["all"]["percentage"]          nested
          win_rate        "team-wins"          value["all"]["percentage"]          nested
          pass_accuracy   NOT IN TEAM STATS    fetched separately from fixtures
        """
        stats = team_data.get("statistics", [])
        if not stats:
            return None

        # Season-filtered: first (and only) entry
        ...
        for detail in details:
            ...
            if not isinstance(type_info, dict):
                # Without .type sub-include, type_info is an integer ID — unusable
                continue
        ...
        # ── Flat structure: value = {"average": X, ...} ───────────────────────
        features["possession"] = float(
            parsed.get("ball-possession", {}).get("average", 50.0)
        )
        features["shots"] = float(
            parsed.get("shots", {}).get("average", 12.0)
        )
        features["tackles"] = float(
            parsed.get("tackles", {}).get("average", 15.0)
        )

        # ── Interceptions: value = {"interceptions_per_game": X, ...} ─────────
        features["interceptions"] = float(
            parsed.get("interception-stats", {}).get("interceptions_per_game", 10.0)
        )

        # ── Nested structure: value = {"all": {"average": X}, ...} ───────────
        features["goals_scored"] = float(
            parsed.get("goals", {}).get("all", {}).get("average", 1.5)
        )
        features["goals_conceded"] = float(
            parsed.get("goals-conceded", {}).get("all", {}).get("average", 1.2)
        )

        # ── Nested structure: value = {"all": {"percentage": X}, ...} ─────────
        features["clean_sheet_pct"] = float(
            parsed.get("cleansheets", {}).get("all", {}).get("percentage", 30.0)
        )

        # ── Win rate: code is "team-wins" (NOT "wins") ────────────────────────
        features["win_rate"] = float(
            parsed.get("team-wins", {}).get("all", {}).get("percentage", 33.0)
        )

        # pass_accuracy is NOT available from this endpoint.
